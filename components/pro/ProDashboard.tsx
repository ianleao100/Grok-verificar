...
import { ShoppingBag, Utensils, DollarSign, Users, LogOut, History, Store, ChevronLeft, ChevronRight, Bell } from 'lucide-react';
...
import { ProTab, OrderStatus } from '../../types';
...
export const ProDashboard: React.FC<ProDashboardProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
      ...
      window.addEventListener('storage-update', checkOrders);
      const interval = setInterval(checkOrders, 10000);

      return () => {
          window.removeEventListener('storage-update', checkOrders);
    ...
    <div className="flex h-screen bg-[#f8f6f6] dark:bg-background-dark overflow-hidden font-display transition-colors">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative h-full overflow-hidden">
        ...
        <div className="absolute top-4 right-8 z-50 flex items-center gap-2">
            <button 
                onClick={playAlert}
                className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-700 text-slate-500 hover:text-[#EA2831] px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-2 shadow-sm transition-all active:scale-95"
            >
