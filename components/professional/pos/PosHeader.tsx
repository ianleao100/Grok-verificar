    ...
    return (
        <header className="h-24 bg-white dark:bg-surface-dark px-10 flex items-center justify-between border-b border-gray-100 dark:border-gray-800 shrink-0">
          ...
          <div className="flex items-center gap-4">
              {selectedTableId && (
                <button onClick={onTransferTable} disabled={!isTransferEnabled} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs uppercase transition-all ${!isTransferEnabled ? 'bg-gray-100 text-slate-400 cursor-not-allowed opacity-60' : 'bg-orange-50 border border-orange-200 text-orange-600 hover:bg-orange-100'}`}>
